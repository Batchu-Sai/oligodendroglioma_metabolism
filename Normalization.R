...
outDir <- file.path("Normalization/")
if(!dir.exists(outDir)) dir.create(outDir,recursive=TRUE)

# load the data

selected_sce <- readRDS("selected_sce.rds")
locations <- unique(selected_sce$Location_General)
...
gene_select_mat <- matrix(FALSE,nrow=nrow(selected_sce),
                          ...
                          dimnames = list(rownames(selected_sce),locations))
for(c in locations){
  each_sce <- subset(selected_sce, select = colData(selected_sce)$Location_General == c)
...
print(sum(rowSums(gene_select_mat) >= length(locations)))
low_dropout_genes <- rownames(gene_select_mat)[rowSums(gene_select_mat) >= length(locations)]
...
all_gene_lengths <- read.table("gene_length.txt",sep="\t",header=F,row.names=1)
...
source("utils.R")
deseq2_sf <- estimateSizeFactorsForMatrix(selected_impute_counts[low_dropout_genes,]+1)
...
scran.sf <- scran::calculateSumFactors(selected_impute_counts[low_dropout_genes,],clusters=selected_sce$Location_General)
summary(scran.sf)
...
all_locations <- as.vector(selected_sce$Location_General)
for(m in c("RLE","TMM","UpperQuartile","Deconvolution"))
{
  ...
  low_dropout_genes_tpm_mean <- apply(low_dropout_genes_tpm, 1, function(x) by(x, all_locations, mean))
